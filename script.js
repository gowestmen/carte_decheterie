  ...
  .then(data => {
    ...
    map.on('load', () => {
      ...
      map.addLayer({
        ...
        "paint": {"line-color": "#228022", "line-width": 1},
      });
    ...
    function buildLocationList(stores) {
      for (const store of stores.features) {
        ...
        listing.id = `listing-${store.properties.nom_court}`;
        ...
        link.id = `link-${store.properties.nom_court}`;
        ...
        if (store.properties.ann_ouv) {
          details.innerHTML += `${" | Année d'ouverture : " + store.properties.ann_ouv}`;
        }
        ...
        link.addEventListener('click', function () {
          for (const feature of stores.features) {
            if (this.id === `link-${feature.properties.nom_court}`) {
              flyToStore(feature);
              createPopUp(feature);
            }
          }
   ...
   function createPopUp(currentFeature) {
      ...
      const horaires = currentFeature.properties.horaires.replace(/\s*\/\s*/g, '<br>'); // Remplacer les slashs par des balises <br>
      const popup = new maplibregl.Popup({ closeOnClick: true})
        .setLngLat(currentFeature.geometry.coordinates)
        .setHTML(
          `<h3>${currentFeature.properties.nom_court}</h3><h4>${horaires}</h4>`
        )
        .addTo(map);
    }
  })
